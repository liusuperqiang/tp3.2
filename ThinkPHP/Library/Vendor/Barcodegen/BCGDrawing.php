    ...
    public function finish($image_style = self::IMG_FORMAT_PNG, $quality = 100) {
        ...
        if ($image_style === self::IMG_FORMAT_PNG) {
            if ($this->filename === null || $this->filename === ''){
                $tempimg_width = imagesx($im);
                $tempimg_height = imagesy($im);

                $imgpnl = imagecreatetruecolor($tempimg_width+20,$tempimg_height+20);
                $color = imagecolorallocate($imgpnl, 255, 255, 255);
                imagefill($imgpnl, 0, 0, $color);

                imagecopyresampled($imgpnl,$im,10,10,0,0,$tempimg_width,$tempimg_height,$tempimg_width,$tempimg_height);
                $im = imagepng($imgpnl);
                imagedestroy($imgpnl);
